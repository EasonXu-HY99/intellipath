...
import { Users as UsersIcon, UserPlus, Trash2, ShieldCheck } from "lucide-react";
        ...
        <GlassCard className="p-6 mb-6" glow>
          <form onSubmit={createUser} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end gap-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="text-sm text-slate-400 hover:text-white px-2">
                Cancel
              </button>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[720px]">
              <thead className="bg-white/5 text-slate-400">
                <tr>
                  {["User", "Role", "Email", "Created", "Actions"].map((h) => (
              ...
              <tbody>
                {(users.data?.users || []).map((u) => (
                  <tr key={u.id} className="border-t border-white/5 align-middle">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        {Number(u.id) === Number(me?.id) && (
                          <span className="text-[10px] text-cyan-300 border border-cyan-400/20 rounded-full px-2 py-0.5">You</span>
                        )}
                      </div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-slate-500 text-xs">{u.created_at}</td>
                    <td className="p-4">
                      {Number(u.id) !== Number(me?.id) ? (
                        <button
                        ...
                        <span className="text-xs text-slate-600">Current account</span>
                      )}
                    </td>
                  </tr>
                ))}
