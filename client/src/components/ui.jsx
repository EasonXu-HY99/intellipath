...
export function GlassCard({ children, className = "", glow = false }) {
  return (
    <div
      className={`glass rounded-2xl ${glow ? "flow-border" : ""} ${className}`}
    >
      {children}
    </div>
  );
...
export function IconBox({ icon: Icon, tone = "neutral", className = "" }) {
  const tones = {
    ...
    violet: "bg-violet-400/10 text-blue-700 border-violet-400/20",
...
const TONE = {
  ...
  amber: "bg-amber-400/10 text-amber-800 border-amber-400/20",
  rose: "bg-rose-400/10 text-rose-700 border-rose-400/20",
  sky: "bg-sky-400/10 text-sky-700 border-sky-400/20",
...
export function RiskBadge({ risk }) {
  ...
  return (
    <span
      className={`inline-flex px-2.5 py-0.5 rounded-full text-xs border ${TONE[tone]}`}
    >
      {risk}
    </span>
  );
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
      <div className="flex items-start gap-4">
        {Icon && (
          <div className="w-12 h-12 rounded-2xl glass flex items-center justify-center text-cyan-700 shrink-0 mt-0.5">
        ...
        <div>
          ...
          <p className="text-slate-600 mt-1 max-w-4xl text-sm">{subtitle}</p>
        </div>
      </div>
      {action}
    </div>
...
export function ErrorBox({ error, onRetry }) {
  return (
    <div className="p-5 rounded-2xl bg-rose-400/10 border border-rose-400/20 text-rose-700">
      <p className="font-semibold flex items-center gap-2">
        <AlertTriangle className="w-4 h-4" /> Failed to load data
      </p>
      <p className="text-sm mt-1 text-rose-700/80">
        {String(error?.message || error)}
      </p>
      {onRetry && (
        <button
          onClick={onRetry}
          className="mt-3 px-3 py-1.5 rounded-lg bg-rose-400/20 text-rose-700 text-sm hover:bg-rose-400/30"
        >
          Retry
        </button>
      )}
...
export function Toast({ message, kind = "success", onDismiss }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-xl glass text-slate-900 shadow-xl">
      <span
        className={`w-2 h-2 rounded-full ${kind === "error" ? "bg-rose-400" : "bg-emerald-400"}`}
      />
...
export function PrimaryButton({ children, onClick, disabled, className = "" }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`px-4 py-2 rounded-xl text-sm font-medium text-slate-900 brand-fill disabled:opacity-50 disabled:cursor-not-allowed transition shadow-lg shadow-cyan-500/10 ${className}`}
    >
      {children}
    </button>
  );
...
export function GhostButton({ children, onClick, disabled, className = "" }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`px-4 py-2 rounded-xl text-sm font-medium text-slate-700 glass-soft hover:bg-slate-100 disabled:opacity-50 transition ${className}`}
    >
      {children}
    </button>
  );
...
export const fieldCls =
  "field w-full px-3 py-2 rounded-xl text-sm placeholder:text-slate-500";
